...
# w_sel = 1  # 0 -> 4
# win_len = wlens[w_sel]
for win_len in wlens:
    electrodes = ''
    descriptor = 'time_domain_hpf'  # FIXED + TRUNCATED
    subject_number = 5
    ...
    TEST_BATCH_SIZE = 100
    LEARNING_RATE = 1e-4  # 'Step size' on n-D optimization plane
    ...
    STRIDE_CONV2D_7 = [1, 1, 2, 1]
    STRIDE_CONV2D_8 = [1, 1, 1, 1]
    ...
    BIAS_VAR_CL5 = 96  # Number of kernel convolutions in h_conv1
    BIAS_VAR_CL6 = 96  # Number of kernel convolutions in h_conv2
    BIAS_VAR_CL7 = 128  # Number of kernel convolutions in h_conv2
    BIAS_VAR_CL8 = 128  # Number of kernel convolutions in h_conv2

    WEIGHT_VAR_CL1 = [1, 8, 1, BIAS_VAR_CL1]  # # [filter_height, filter_width, in_channels, out_channels]
    WEIGHT_VAR_CL2 = [1, 8, BIAS_VAR_CL1, BIAS_VAR_CL2]  # # [filter_height, filter_width, in_channels, out_channels]
    WEIGHT_VAR_CL3 = [1, 8, BIAS_VAR_CL2, BIAS_VAR_CL3]
    WEIGHT_VAR_CL4 = [1, 8, BIAS_VAR_CL3, BIAS_VAR_CL4]

    WEIGHT_VAR_CL5 = [1, 4, BIAS_VAR_CL4, BIAS_VAR_CL5]
    WEIGHT_VAR_CL6 = [1, 4, BIAS_VAR_CL5, BIAS_VAR_CL6]
    WEIGHT_VAR_CL7 = [1, 2, BIAS_VAR_CL6, BIAS_VAR_CL7]
    WEIGHT_VAR_CL8 = [1, 2, BIAS_VAR_CL7, BIAS_VAR_CL8]
    ...
    MAX_POOL_FLAT_SHAPE_FC1 = [-1, NUMBER_DATA_CHANNELS * (DATA_WINDOW_SIZE // DIVIDER) * BIAS_VAR_CL8]
    ...
    def leaky_conv2d_w_dropout(x_, w_, b_, stride, alpha, keep_prob_):
        x_ = tf.nn.conv2d(x_, w_, strides=stride, padding='SAME')
        x_ = tf.nn.bias_add(x_, b_)
        x_ = tf.nn.leaky_relu(x_, alpha=alpha)
        return tf.nn.dropout(x_, keep_prob=keep_prob_)
    ...
    # TODO TRY:
    # h_conv1 = leaky_conv2d_w_dropout(x_input, W_conv1, b_conv1, STRIDE_CONV2D_1, alpha=ALPHA_C1, keep_prob=0.1)
    ...
    # h_conv1 = leaky_conv2d_w_dropout(x_input, W_conv1, b_conv1, STRIDE_CONV2D_1, alpha=ALPHA_C1, keep_prob_=drop_c1)
    ...
    W_conv5 = weight_variable(WEIGHT_VAR_CL5)
    b_conv5 = bias_variable([BIAS_VAR_CL5])
    h_conv5 = leaky_conv2d(h_conv4, W_conv5, b_conv5, STRIDE_CONV2D_5, alpha=ALPHA_C5)

    W_conv6 = weight_variable(WEIGHT_VAR_CL6)
    b_conv6 = bias_variable([BIAS_VAR_CL6])
    h_conv6 = leaky_conv2d(h_conv5, W_conv6, b_conv6, STRIDE_CONV2D_6, alpha=ALPHA_C6)

    W_conv7 = weight_variable(WEIGHT_VAR_CL7)
    b_conv7 = bias_variable([BIAS_VAR_CL7])
    h_conv7 = leaky_conv2d(h_conv6, W_conv7, b_conv7, STRIDE_CONV2D_7, alpha=ALPHA_C7)

    W_conv8 = weight_variable(WEIGHT_VAR_CL8)
    b_conv8 = bias_variable([BIAS_VAR_CL8])
    h_conv8 = leaky_conv2d(h_conv7, W_conv8, b_conv8, STRIDE_CONV2D_8, alpha=ALPHA_C8)

    # the input should be shaped/flattened
    h_conv_flat = tf.reshape(h_conv8, MAX_POOL_FLAT_SHAPE_FC1)
    ...
    # TODO: softmax_cross_entropy_with_logits is deprecated.
    # TODO: Future major versions of TensorFlow will allow gradients to flow
    # TODO: into the labels input on backprop by default. See tf.nn.softmax_cross_entropy_with_logits_v2.
    ...
    with tf.Session(config=config) as sess:
        ...
        h_conv5_shape = sess.run(h_conv5, feed_dict={x: x_0, keep_prob: 1.0}).shape
        h_conv6_shape = sess.run(h_conv6, feed_dict={x: x_0, keep_prob: 1.0}).shape
        h_conv7_shape = sess.run(h_conv7, feed_dict={x: x_0, keep_prob: 1.0}).shape
        h_conv8_shape = sess.run(h_conv8, feed_dict={x: x_0, keep_prob: 1.0}).shape
        ...
        print("h_conv5: ", h_conv5_shape)
        print("h_conv6: ", h_conv6_shape)
        print("h_conv7: ", h_conv7_shape)
        print("h_conv8: ", h_conv8_shape)
        ...
        print("Subject #", subject_number, " wlen: ", win_len)
